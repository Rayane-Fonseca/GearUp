...
test('aluno nao pode emitir certificado se nao tiver concluido 100% do curso', function () {
    ...
    $idCurso = (int) ($curso->id_curso ?? $curso->id);

    // Persiste o usuário no banco para evitar inconsistências de instância
    $usuario = Usuario::create([
        'nome' => 'Aluno Teste',
        'email' => 'aluno.teste@gearup.com',
        'password' => bcrypt('password'),
        'perfil' => 'colaborador',
    ]);

    // Aplica o mock na própria instância salva
    $mockUsuario = Mockery::mock($usuario)->makePartial();
    $mockUsuario->shouldReceive('progressoNoCurso')
        ->andReturn(50.00);

    expect(fn () => Certificado::validarEmissao((int) $mockUsuario->getKey(), $idCurso, $mockUsuario))
...
test('aluno consegue validar emissao si tiver concluido 100% do curso', function () {
    ...
    $progresso = $mockUsuario->progressoNoCurso($idCurso);
