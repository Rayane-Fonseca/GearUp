        ...
        return $form
            ->schema([
                Forms\Components\Section::make('Cadastro de Aula')
                    ->schema([
                        // Campo auxiliar para filtrar os módulos por Curso
                        Select::make('id_curso_auxiliar')
                            ->label('Selecione o Curso (Filtro)')
                            ->options(Curso::pluck('titulo', 'id_curso'))
                            ->searchable()
                            ->live()
                            ->afterStateUpdated(fn (callable $set) => $set('id_modulo', null))
                            ->dehydrated(false), // Impede de enviar este campo fake para o banco

                        // Campo real gravado na tabela 'aulas'
                        Select::make('id_modulo')
                            ->label('Módulo Correspondente')
                            ->options(function (callable $get, ?Aula $record) {
                                $cursoId = $get('id_curso_auxiliar');

                                // Se selecionou um curso no filtro
                                if ($cursoId) {
                                    return Modulo::where('id_curso', $cursoId)->pluck('titulo', 'id_modulo');
                                }

                                // Se estiver Editando, descobre o curso a partir da aula gravada
                                if ($record && $record->modulo) {
                                    return Modulo::where('id_curso', $record->modulo->id_curso)->pluck('titulo', 'id_modulo');
                                }

                                // Padrão: lista todos os módulos
                                return Modulo::pluck('titulo', 'id_modulo');
                            })
                            ->searchable()
                            ->live()
                            ->required(),
