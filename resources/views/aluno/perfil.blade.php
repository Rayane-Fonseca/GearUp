<x-aluno-layout titulo-pagina="Perfil" subtitulo-pagina="Suas informações e configurações">
    <div x-data="{ aba: '{{ session('status') || session('success') || $errors->any() || $errors->updatePassword->any() ? 'senha' : 'perfil' }}' }" class="p-8 max-w-7xl mx-auto space-y-6">
        <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="md:col-span-1 space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    <div class="p-5 -mt-10">
                        <div class="w-16 h-16 rounded-full bg-blue-600 border-4 border-white flex items-center justify-center text-white font-bold text-lg">{{ $usuario->iniciais() }}</div>
                        <h3 class="font-bold text-gray-900 mt-3">{{ $usuario->nome }}</h3>
                        <p class="text-xs text-gray-500">{{ $usuario->cargo }}</p>
                        <p class="text-xs text-gray-400">{{ $usuario->area }}</p>

                        <div class="grid grid-cols-3 gap-2 mt-5 text-center">
                            <div class="bg-gray-50 rounded-xl p-2.5">
                                <p class="font-bold text-gray-900">{{ $cursosCount }}</p>
                                <p class="text-[10px] text-gray-400">Cursos</p>
                            </div>
                            <div class="bg-gray-50 rounded-xl p-2.5">
                                <p class="font-bold text-gray-900">{{ $horasTotais }}h</p>
                                <p class="text-[10px] text-gray-400">Horas</p>
                            </div>
                            <div class="bg-gray-50 rounded-xl p-2.5">
                                <p class="font-bold text-gray-900">{{ $certificadosCount }}</p>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm divide-y divide-gray-100 overflow-hidden">
                    ...
                    <a href="{{ route('aluno.notificacoes') }}" class="flex items-center justify-between p-4 text-xs font-medium text-gray-700 hover:bg-gray-50">
            ...
            <div class="md:col-span-2 space-y-6">
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                        ...
                        <div class="grid grid-cols-2 gap-4 text-xs">
                            <div class="bg-gray-50 rounded-xl p-3">
                                ...
                                <p class="font-semibold text-gray-800 mt-0.5">{{ $usuario->cargo }}</p>
                            </div>
                            <div class="bg-gray-50 rounded-xl p-3">
                            ...
                            <div class="bg-gray-50 rounded-xl p-3">
                                ...
                                <p class="font-semibold text-gray-800 mt-0.5">{{ $usuario->area }}</p>
                            </div>
                            <div class="bg-gray-50 rounded-xl p-3">
                                <p class="text-gray-400">E-mail</p>
                                <p class="font-semibold text-gray-800 mt-0.5">{{ $usuario->email }}</p>
                            </div>
                        </div>
                    </div>

                    {{-- PROGRESSO POR ÁREA COM CORES DINÂMICAS --}}
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                        <h4 class="font-bold text-gray-900 mb-4">Progresso por área</h4>
                        <div class="space-y-4">
                            @forelse($progressoPorArea as $area => $percentual)
                            @php
                            $corBarra = match($area) {
                            'DevOps' => '#9B5DE5',
                            ...
                            <div>
                                <div class="flex justify-between text-xs font-medium mb-1">
                                    ...
                                    <span class="text-gray-500">{{ $percentual }}%</span>
                                </div>
                ...
                <div x-show="aba === 'senha'" x-cloak class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-6">
                    ...
                    @if (session('status') === 'password-updated' || session('status'))
